...
export class TodoService {
  ...
  readonly doneCount = computed(() => this.todos().filter((item) => item.done).length);

  readonly openCount = computed(() => this.todos().filter((item) => !item.done).length);

  readonly sortedTodos = computed(() => this.todos().sort((b, a) => +b.done - +a.done));
